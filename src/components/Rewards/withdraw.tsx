...
import { withdrawLP } from "../../hooks/Rewards";
...
export default (props: any) => {
  ...
  const [result, setResult] = useState<{lpBalance: string, rewardUnlockedUser: string, rewardEstimate: string, rewardTotal:string, rewardUnlocked: string }>({lpBalance: '0', rewardUnlockedUser: '0', rewardEstimate: '0', rewardTotal: '0', rewardUnlocked: '0' })
  const [estimate, setEstimate] = useState('0')

  const {  library } = useActiveWeb3React()

  function selectPercentage(amount: number) {
    let calculated = ((Number(result.lpBalance) * amount) / 100)
    let rewards = Number(result.rewardEstimate) - (Number(result.rewardEstimate) * amount / 100)
  ...
  useEffect(() => {
    ...
    return (
      <Wrapper>
        <Wrapper2>
          <Text>Balance</Text> <Balance><span>{result.lpBalance} </span> &nbsp; <span>{contract.token0 + '-' +  contract.token1}</span></Balance>
         </Wrapper2>
        <InputWrapper>
        ...
        <ButtonPrimary onClick={() => withdrawLP(contract.stakingContractAddress, contract.tokenAddress, contract.user, library, withdrawValue)}> Withdraw LP Tokens</ButtonPrimary>
      </Wrapper>
